                ...
                Row (
                    ...
                    Box(
                        ...
                        DropdownMenu(
                            ...
                            com.adamnoorapps.recipefinder.data.Unit.entries.forEach { enumUnit ->
                                DropdownMenuItem(
            ...
            Button(
                ...
                onClick = {
                    if (name.isNotBlank() && quantity > 0 && unit.isNotBlank()) {
                        onConfirm(Ingredient(0, name.lowercase().replaceFirstChar(Char::titlecase), quantity.toInt(), com.adamnoorapps.recipefinder.data.Unit.valueOf(unit.uppercase()), note))
                    }
                }
            ) {
                Text("Confirm")
    ...
    androidx.compose.material3.AlertDialog(
        ...
        confirmButton = {
            Button(
                ...
                onClick = {onConfirm(Ingredient(ingredient.id, newName.value.lowercase().replaceFirstChar(Char::titlecase), newQuantity.intValue, com.adamnoorapps.recipefinder.data.Unit.valueOf(newUnit.value.uppercase())))}
            ) {
                Text("Confirm")
            }
        },
